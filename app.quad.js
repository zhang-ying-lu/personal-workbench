...
(function () {
  ...
  /* ---- 资源库：艾森豪威尔矩阵（四象限坐标系 + 右下角输入区） ----
     象限骨架（轴标签 + 4 cell + 十字线）写在 index.html，JS 只渲染右下角「少去做」输入区 */
  const EISEN_KEY = "q3"; // 写入到 store.eisenhower 的右下角象限 key
  function renderEisenhower() {
    const board = $("#eisenBoard");
    if (!board) return;
    const items = store.eisenhower[EISEN_KEY] || [];
    const lis = items.length ? items.map((it) =>
      '<li class="eisen-item' + (it.done ? " done" : "") + '" data-k="' + EISEN_KEY + '" data-id="' + it.id + '">' +
    ...
    board.innerHTML =
      '<div class="eisen-add">' +
        '<input type="text" class="inp" data-k="' + EISEN_KEY + '" placeholder="紧急但不重要的事，回车添加…" />' +
        '<button class="eisen-add-btn" data-k="' + EISEN_KEY + '" title="添加">＋</button>' +
      '</div>' +
