...
(function () {
  ...
  /* ---- 资源库：艾森豪威尔矩阵（紧急 × 重要 四象限） ----
     渲染顺序即矩阵方位：左上(重要不紧急) 右上(重要紧急) 左下(不重要不紧急) 右下(紧急不重要)
     EISEN_SHOW 控制展示哪些象限；当前按用户要求只保留右下角「委托做」 */
  const EISEN_QS = [
    { k: "q2", title: "重要 · 不紧急", sub: "计划做", hint: "安排时间，稳步推进" },
    { k: "q1", title: "重要 · 紧急", sub: "马上做", hint: "立即处理，优先完成" },
    { k: "q4", title: "不重要 · 不紧急", sub: "尽量不做", hint: "放下它，别浪费精力" },
    { k: "q3", title: "紧急 · 不重要", sub: "委托做", hint: "少亲自做，交给别人" }
  ];
  const EISEN_SHOW = ["q3"];
  function renderEisenhower() {
    const board = $("#eisenBoard");
    if (!board) return;
    const solo = EISEN_SHOW.length === 1;
    board.innerHTML = EISEN_QS.filter((q) => EISEN_SHOW.includes(q.k)).map((q) => {
      ...
      const lis = items.length ? items.map((it) =>
        ...
        '</li>').join("")
        : '<li class="eisen-empty">还没有事项，加一件吧</li>';
      return '<div class="eisen-cell' + (solo ? " solo" : "") + '">' +
        '<div class="eisen-cell-head">' +
          '<span class="eisen-dot"></span>' +
          '<span class="eisen-title">' + q.title + '</span>' +
          '<span class="eisen-sub">' + q.sub + '</span>' +
        '</div>' +
        '<div class="eisen-add">' +
    ...
    board.querySelectorAll(".eisen-add-btn").forEach((b) => b.addEventListener("click", () => addEisen(b.dataset.k)));
    board.querySelectorAll(".eisen-add input").forEach((inp) => inp.addEventListener("keydown", (e) => { if (e.key === "Enter") addEisen(inp.dataset.k); }));
    board.querySelectorAll(".eisen-item").forEach((li) => {
      li.addEventListener("click", (e) => {
        if (e.target.classList.contains("eisen-del")) return;
        toggleEisen(li.dataset.k, li.dataset.id);
      });
    });
    board.querySelectorAll(".eisen-del").forEach((b) => {
  ...
  function addEisen(k) {
    const inp = document.querySelector('#eisenBoard .eisen-add input[data-k="' + k + '"]');
