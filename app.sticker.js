...
(function () {
  ...
  function emoFaceSVG(box, size) {
    ...
    if (!icon) {
      const e = (box && box.emoji) || "🙂";
      return '<span style="font-size:' + Math.round(size * 0.55) + 'px;line-height:1">' + e + '</span>';
    }
    const c = esc(box.color || "#f6c453");
    const ink = "#43301f";
    const pink = "#ff9bb0";
    const cx = 24, cy = 26, R = 14.5;
    // 贴纸白边：耳朵+头 白色描边形成 die-cut 边缘
    const back =
      '<path d="M12 17 L9 3 L23 13 Z" fill="#fff" stroke="#fff" stroke-width="6" stroke-linejoin="round"/>' +
      '<path d="M36 17 L39 3 L25 13 Z" fill="#fff" stroke="#fff" stroke-width="6" stroke-linejoin="round"/>' +
      '<circle cx="' + cx + '" cy="' + cy + '" r="' + R + '" fill="#fff" stroke="#fff" stroke-width="6"/>';
    // 彩色层（粗黑描边、纯色扁平填充）
    const ears =
      '<path d="M12 17 L9 3 L23 13 Z" fill="' + c + '" stroke="' + ink + '" stroke-width="2.6" stroke-linejoin="round"/>' +
      '<path d="M36 17 L39 3 L25 13 Z" fill="' + c + '" stroke="' + ink + '" stroke-width="2.6" stroke-linejoin="round"/>' +
      '<path d="M14.5 14 L12.5 6.5 L20 12.8 Z" fill="' + pink + '"/>' +
      '<path d="M33.5 14 L35.5 6.5 L28 12.8 Z" fill="' + pink + '"/>';
    const head = '<circle cx="' + cx + '" cy="' + cy + '" r="' + R + '" fill="' + c + '" stroke="' + ink + '" stroke-width="2.6"/>';
    const blush = '<g fill="' + pink + '"><ellipse cx="13" cy="32" rx="3.4" ry="2.1"/><ellipse cx="35" cy="32" rx="3.4" ry="2.1"/></g>';
    const nose = '<path d="M22.6 30 L25.4 30 L24 32 Z" fill="' + pink + '" stroke="' + ink + '" stroke-width="0.8" stroke-linejoin="round"/>';
    const whiskers = '<g stroke="' + ink + '" stroke-width="1.3" stroke-linecap="round" opacity=".7">' +
      '<path d="M10 30 H4"/><path d="M10 33 H4"/>' +
      '<path d="M38 30 H44"/><path d="M38 33 H44"/></g>';
    const mouth = '<path d="M24 32 Q21.5 35 18.5 32.5 M24 32 Q26.5 35 29.5 32.5" fill="none" stroke="' + ink + '" stroke-width="1.8" stroke-linecap="round"/>';
    const eye = (ex, r) =>
      '<circle cx="' + ex + '" cy="27" r="' + r + '" fill="' + ink + '"/>' +
      '<circle cx="' + (ex - r * 0.32) + '" cy="' + (27 - r * 0.4) + '" r="' + (r * 0.34) + '" fill="#fff"/>';
    let face = "", cue = "";
    if (icon === "happy") {
      face = '<path d="M13 28 Q17 22 21 28" fill="none" stroke="' + ink + '" stroke-width="2.6" stroke-linecap="round"/>' +
        '<path d="M27 28 Q31 22 35 28" fill="none" stroke="' + ink + '" stroke-width="2.6" stroke-linecap="round"/>' +
        '<path d="M18 33 Q24 40 30 33" fill="none" stroke="' + ink + '" stroke-width="2.4" stroke-linecap="round"/>';
    } else if (icon === "sad") {
      face = eye(17, 3) + eye(31, 3) + '<path d="M19 35 Q24 31.5 29 35" fill="none" stroke="' + ink + '" stroke-width="2.2" stroke-linecap="round"/>';
      cue = '<path d="M13 28 Q10.5 33 13.5 37.5 Q16.5 33 15 28 Z" fill="#7fb4e8" stroke="' + ink + '" stroke-width="1.2"/>';
    } else if (icon === "wronged") {
      face = eye(17, 2.8) + eye(31, 2.8) +
        '<path d="M13.5 20 L19 23" stroke="' + ink + '" stroke-width="2" stroke-linecap="round"/>' +
        '<path d="M34.5 20 L29 23" stroke="' + ink + '" stroke-width="2" stroke-linecap="round"/>' +
        '<path d="M21 35 Q24 33 27 35" fill="none" stroke="' + ink + '" stroke-width="2" stroke-linecap="round"/>';
      cue = '<path d="M30 28 Q27.5 33 30.5 37.5 Q33.5 33 32 28 Z" fill="#9bc4ea" stroke="' + ink + '" stroke-width="1.2"/>';
    } else if (icon === "anxious") {
      face = eye(17, 3.3) + eye(31, 3.3) +
        '<path d="M11 18 Q17 14.5 21 18" fill="none" stroke="' + ink + '" stroke-width="1.8" stroke-linecap="round"/>' +
        '<path d="M27 18 Q31 14.5 37 18" fill="none" stroke="' + ink + '" stroke-width="1.8" stroke-linecap="round"/>' +
        '<path d="M16 35 Q19 32 22 35 Q25 38 28 35 Q31 32 34 35" fill="none" stroke="' + ink + '" stroke-width="2" stroke-linecap="round"/>';
      cue = '<path d="M41 10 Q44 16 41 19.5 Q38 16 41 10 Z" fill="#7fb4e8" stroke="' + ink + '" stroke-width="1.2"/>';
    } else if (icon === "tired") {
      face = '<path d="M13 27 Q17 31 21 27" fill="none" stroke="' + ink + '" stroke-width="2.6" stroke-linecap="round"/>' +
        '<path d="M27 27 Q31 31 35 27" fill="none" stroke="' + ink + '" stroke-width="2.6" stroke-linecap="round"/>' +
        '<ellipse cx="24" cy="35.5" rx="3.4" ry="4" fill="' + ink + '"/>';
      cue = '<text x="36" y="16" font-size="10" fill="#9ab87a" font-family="serif" font-style="italic">Z</text>';
    } else if (icon === "lost") {
      face = '<circle cx="18.5" cy="27" r="1.9" fill="' + ink + '"/><circle cx="29.5" cy="27" r="1.9" fill="' + ink + '"/>' +
        '<path d="M16 35 Q19 32.5 22 35 Q25 37.5 28 35 Q31 32.5 34 35" fill="none" stroke="' + ink + '" stroke-width="2" stroke-linecap="round"/>';
      cue = '<text x="35" y="16" font-size="13" fill="' + ink + '" font-family="serif" font-style="italic">?</text>';
    } else {
      face = eye(17, 3) + eye(31, 3) + mouth;
    }
    return '<svg viewBox="0 0 48 48" width="' + size + '" height="' + size + '" aria-hidden="true" style="display:block">' +
      back + ears + head + whiskers + blush + nose + face + cue + '</svg>';
  }
